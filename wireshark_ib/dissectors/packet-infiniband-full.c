...
#ifdef MLNX_INT_DCT
/* DC Transport Header (DCETH) */
static int hf_infiniband_dct_dci = -1;
static int hf_infiniband_fecn = -1;
static int hf_infiniband_becn = -1;
static int hf_infiniband_sid_owner = -1;
static int hf_infiniband_sid = -1;
static int hf_infiniband_dci_dct = -1;
/* DC Connect Extended Header (DCCETH) */
static int hf_infiniband_DCCETH = -1;
static int hf_infiniband_reconnect = -1;
static int hf_infiniband_dct_access_key = -1;
#endif
/* Raw Header (RWH) */
...
static const value_string OpCodeMap[] =
{
    ...
    { RD_RESYNC,                    "DC Connect/Disconnect "},
#else
...
#ifdef MLNX_INT_DCT
            if (dctBthHeader) {
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_fecn,                     tvb, offset, 1, ENC_BIG_ENDIAN);
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_becn,                     tvb, offset, 1, ENC_BIG_ENDIAN); offset += 1;
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_dct_dci,                  tvb, offset, 3, ENC_BIG_ENDIAN);
            }
            else {
...
#ifdef MLNX_INT_DCT
            if (dctBthHeader) {
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_sid_owner,                    tvb, offset, 1, ENC_BIG_ENDIAN); offset += 1;
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_sid,                          tvb, offset, 3, ENC_BIG_ENDIAN); offset += 4;
                proto_tree_add_item(base_transport_header_tree, hf_infiniband_dci_dct,                      tvb, offset, 1, ENC_BIG_ENDIAN); offset += 3;
            }
#else
            offset += bthSize - 12;
#endif
...
parse_DCCETH(proto_tree *parentTree _U_, tvbuff_t *tvb _U_, gint *offset)
{
    ...
    /* DETH - Datagram Extended Transport Header */
    gint        local_offset = *offset;
    proto_item *DCCETH_header_item;
    proto_tree *DCCETH_header_tree;

    DCCETH_header_item = proto_tree_add_item(parentTree, hf_infiniband_DCCETH, tvb, local_offset, 16, ENC_NA);
    proto_item_set_text(DCCETH_header_item, "%s", "DCCETH - DC Connect Extended Transport Header");
    DCCETH_header_tree = proto_item_add_subtree(DCCETH_header_item, ett_dcceth);

    proto_tree_add_item(DCCETH_header_tree, hf_infiniband_reconnect,      tvb, local_offset, 1, ENC_BIG_ENDIAN); local_offset += 4;
    local_offset += 4; /* skip reserved 4 bytes */
    proto_tree_add_item(DCCETH_header_tree, hf_infiniband_dct_access_key, tvb, local_offset, 8, ENC_BIG_ENDIAN); local_offset += 8;
    *offset = local_offset;
#else
...
#ifdef MLNX_INT_DCT
        /* Mlnx DCTEH - DC Extension Header */
        { &hf_infiniband_dct_dci, {
                "DCT for request/DCI for response", "infiniband.bth.dct_dci",
                FT_UINT24, BASE_DEC, NULL, 0x0, NULL, HFILL}
        },
        { &hf_infiniband_fecn, {
                "FECN", "infiniband.bth.fecn",
                FT_BOOLEAN, 8, NULL, 0x80, NULL, HFILL}
        },
        { &hf_infiniband_becn, {
                "BECN", "infiniband.bth.becn",
                FT_BOOLEAN, 8, NULL, 0x40, NULL, HFILL}
        },
        { &hf_infiniband_sid_owner, {
                "SID owner", "infiniband.dcth.sid_owner",
                FT_BOOLEAN, 8, NULL, 0x1, NULL, HFILL}
        },
        { &hf_infiniband_sid, {
                "Session ID", "infiniband.dcth.sid",
                FT_UINT24, BASE_DEC, NULL, 0, NULL, HFILL}
        },
        { &hf_infiniband_dci_dct, {
                "DCI for request/DCT for response", "infiniband.dceth.dci_dct",
                FT_UINT24, BASE_DEC, NULL, 0, NULL, HFILL}
        },

        /* Mlnx DCCETH - DC Connect Extended Transport Header */
        { &hf_infiniband_DCCETH, {
                "DC Connect Extended Transport Header", "infiniband.dcceth",
                FT_BYTES, BASE_NONE, NULL, 0x0, NULL, HFILL}
        },
        { &hf_infiniband_reconnect, {
                "Reconnect", "infiniband.dcceth.reconnect",
                FT_BOOLEAN, 8, NULL, 0x10, NULL, HFILL}
        },
        { &hf_infiniband_dct_access_key, {
                "DCT Access Key", "infiniband.dcceth.dct_access_key",
...
#ifdef MLNX_INT_NAMING
    proto_infiniband = proto_register_protocol("Infiniband Mlnx", "IB Mlnx", "infiniband_mlnx");
    ib_handle = register_dissector("infiniband_mlnx", dissect_infiniband, proto_infiniband);
#else
...
#ifdef MLNX_INT_NAMING
    proto_infiniband_link = proto_register_protocol("Infiniband Link Mlnx", "Infiniband Link Mlnx", "infiniband_link_mlnx");
    register_dissector("infiniband_link_mlnx", dissect_infiniband_link, proto_infiniband_link);
#else
...
void proto_reg_handoff_infiniband(void)
{
    ...
    dissector_add_uint("erf.types.type", ERF_TYPE_INFINIBAND_LINK, ib_handle);

    /* create and announce an anonymous RRoCE dissector */
    rroce_handle = create_dissector_handle(dissect_rroce, proto_infiniband);
#ifdef MLNX_INT_ROCE_V1_5
    dissector_add_uint("ip.proto", 0xfe, rroce_handle);
#endif
    if (!initialized)
    {
        initialized = TRUE;
